...
const static int REQUEST_MAX_SIZE = 4096;

/* How much of the file to read at a time. */
const static int BUFFER_SIZE = 4096;

const static char *CRLF = "\r\n";
const static char *REQUEST_TERMINATOR = "\r\n\r\n";
const static char *REQUEST_TERMINATOR_LOOSE = "\n\n";
const static char *SERVER = "Sioux/1.0 (Unix)";
const static char *HTTP_VERSION = "HTTP/1.0";
const static char *INDEX_FILE = "index.html";

static int web_setup_socket(int port);
static int web_next_connection(int listen_socket);
static void web_handle_connection(int conn, const char *docroot);
...
int web_setup_socket(int port) {
	...
	listen_addr.sin_port = htons((unsigned short int)port);
...
void web_handle_connection(int conn, const char *docroot) {
	...
	status_t status;
	request_buf = (char*)malloc(sizeof(char)*REQUEST_MAX_SIZE);
	assert(request_buf != NULL);
	filename = (char*)malloc(sizeof(char)*REQUEST_MAX_SIZE);
	...
	status = web_parse_request(request_buf, filename, docroot);
